...
assert.match(source, /\/\/ @version\s+2\.1\.9/);
assert.match(source, /data-fyp-page-ready', '2\.1\.9'/);
...
assert.match(source, /const SEARCH_TRIGGER_LAYOUT_VERSION = 'masthead-slot-v219'/);
assert.match(source, /const NAV_LAYOUT_VERSION = 'ext-v219-masthead-search-settings'/);
...
assert.match(
  source,
  /top: calc\(max\(env\(safe-area-inset-top, 0px\), 20px\) \+ 6px\)/
);
assert.match(
  source,
...
assert.doesNotMatch(source, /mastheadEnd\.insertBefore/);
...
assert.match(source, /const SEARCH_OVERLAY_LAYOUT_VERSION = 'suggest-recents-v219'/);
...
assert.match(template, /EXPECTED_PAGE_VERSION = '2\.1\.9'/);
...
console.log('mobile search masthead-slot trigger + overlay: ok');
